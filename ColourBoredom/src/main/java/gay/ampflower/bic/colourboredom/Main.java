package gay.ampflower.bic.colourboredom;
...
public class Main {
    public static void main(String[] args) throws IOException {
        if(args.length < 4){
            System.out.println(ProcessHandle.current().info().commandLine().orElse("unknown") + " <x> <y> <count> <output>");
            return;
        }
        ...
        var p = FileSystems.getDefault().getPath(args[3]);
